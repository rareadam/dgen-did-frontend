...
import { DidKeyRegistryAbi, DidKeyRegistryAddress, DidAccountLinkRegistryAbi, DidAccountLinkRegistryAddress } from '../contracts';

interface RemoveLinkedAccountButtonProps {
    did: string;
    linkedAccountId: string;
    onRemoveLinkedAccount: () => void;
}

const RemoveLinkedAccountButton = ({ did, onRemoveLinkedAccount, linkedAccountId }: RemoveLinkedAccountButtonProps) => {
    ...
    const handleRemoveLinkedAccount = async () => {
        ...
        writeContract({
            ...
            functionName: 'removeLinkedAccount',
            args: [didAddress as `0x${string}`, linkedAccountId],
        });
    ...
    if (isError) {
        if (isOpen) {
            toast({
                title: 'Failed to Remove Linked Account',
    ...
    return (
        <>
            ...
            <Modal isOpen={isOpen} onClose={onClose} isCentered>
                <ModalOverlay />
                <ModalContent>
                    <ModalHeader>Remove Linked Account</ModalHeader>
                    <ModalCloseButton />
                    <ModalBody pb={6}>
                        <Text>Are you sure you want to remove this linked account?</Text>
                        <Text fontWeight="bold">Account ID:</Text>
                        <Text>{linkedAccountId}</Text>
                    </ModalBody>

                    <ModalFooter>
                        <Button colorScheme="red" mr={3} onClick={handleRemoveLinkedAccount} isLoading={isLoading}>
