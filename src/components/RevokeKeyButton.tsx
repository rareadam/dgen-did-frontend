...
const RevokeKeyButton = ({ did, onRemoveKey, keyId }: RevokeKeyButtonProps) => {
    ...
    const handleRemoveKey = async () => {
        ...
        writeContract({
            ...
            args: [didAddress as `0x${string}`, keyId],
        });
    ...
    if (isSuccess) {
        if (isOpen) {
            toast({
                title: 'Key Removed',
                description: "The key has been successfully removed.",
                status: 'success',
                duration: 9000,
                isClosable: true,
            });
    ...
    return (
        <>
            ...
            <Modal isOpen={isOpen} onClose={onClose} isCentered>
                <ModalOverlay />
                <ModalContent>
                    <ModalHeader>Remove Key</ModalHeader>
                    <ModalCloseButton />
                    <ModalBody pb={6}>
                        <Text>Are you sure you want to remove this key?</Text>
                        <Text fontWeight="bold">Key ID:</Text>
                        <Text>{keyId}</Text>
                    </ModalBody>

                    <ModalFooter>
                        <Button colorScheme="red" mr={3} onClick={handleRemoveKey} isLoading={isLoading}>
                            Remove Key
                        </Button>
...
export default RevokeKeyButton;
